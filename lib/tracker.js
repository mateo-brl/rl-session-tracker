...
async function doInit() {
  ...
  // Warmup des N pages EN PARALLÈLE (sinon ~N×30 s au démarrage).
  const slots = await Promise.all(
    Array.from({ length: POOL_SIZE }, async () => {
      try { return { page: await makeWarmPage(b), busy: false }; }
      catch (e) { return null; }
    })
  );
...
// Recharge la page d'un slot (Cloudflare périmé, ou page morte).
