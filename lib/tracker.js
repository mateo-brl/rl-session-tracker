...
// Récupère le profil d'un joueur (MMR / rang). Lève si la plateforme est
// inconnue. Depuis la v2.3, le détail des matchs vient de l'agent (API du
// jeu) : on n'interroge donc plus l'endpoint « sessions » de tracker.gg.
async function scrapeProfile(platform, username) {
  ...
  try {
    let res = await withTimeout(
      fetchInPage(slot.page, [profileUrl]), FETCH_TIMEOUT, 'scrape');
