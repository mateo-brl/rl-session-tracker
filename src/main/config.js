...
// Nettoyage d'une disposition (dashboard ou overlay) : positions bornées,
// réglages par bloc bornés eux aussi. Une seule implémentation pour les deux,
// sinon l'une des deux dérive au premier ajout.
function sanitizeLayout(src) {
  const out = {};
  for (const id of Object.keys(src).slice(0, 12)) {
    const w = src[id];
    ...
    // Réglages propres au bloc (quels modes afficher, combien de lignes…).
    // Le contenu dépend du bloc : on borne la taille et on laisse la fenêtre
    // décider du sens, sans liste blanche à maintenir en double ici.
...
function update(partial) {
  if (partial && typeof partial === 'object') {
    ...
    if (partial.obsLayout === null) {
      config.obsLayout = null;
    } else if (partial.obsLayout && typeof partial.obsLayout === 'object') {
      const out = sanitizeLayout(partial.obsLayout);
      if (Object.keys(out).length) config.obsLayout = out;
    }
    if (partial.layout === null) {
      config.layouts[config.layoutSlot] = null;
    } else if (partial.layout && typeof partial.layout === 'object') {
      const out = sanitizeLayout(partial.layout);
