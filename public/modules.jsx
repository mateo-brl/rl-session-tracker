...
/* ============================================================
   Data-source badge — d'où viennent les données affichées
============================================================ */

// 🟢 Agent connecté · live  → la Stats API native du jeu alimente le dashboard.
// 🟡 via tracker.gg · différé → repli sur le scraping tracker.gg (quelques min
//    de retard). L'agent est l'unique source temps réel ; sans lui, différé.
function DataSourceBadge() {
  useLang();
  const s = useRLState();
  if (!s) return null;
  const live = !!(s.statsApi && s.statsApi.connected === true);
  return (
    <span
      className={'rl-source-badge ' + (live ? 'is-live' : 'is-deferred')}
      title={live ? T('source.live') : T('source.deferred')}
    >
      <span className="rl-source-dot" />
      <span className="rl-source-label">
        {live ? T('source.live') : T('source.deferred')}
      </span>
    </span>
  );
}
...
Object.assign(window, {
  ...
  StatusPill, DataSourceBadge, RankBadge, PlayerHeader, PlaylistPicker,
