...
| 🎧 **Spotify** | Bloc « En écoute » sur le dashboard et l'overlay : pochette, titre, artiste, barre de progression, et la télécommande — précédent, lecture/pause, suivant, volume. Connexion OAuth avec PKCE, sans secret embarqué : tu crées ton application Spotify, tu colles son identifiant client. **Le son reste joué par ton Spotify** : aucune API ne permet de le diffuser dans une application tierce. Le contrôle demande un compte Premium, l'affichage marche aussi en gratuit. |
...
| 🎧 **Spotify** | A "Now playing" block on the dashboard and overlay: cover, title, artist, progress bar, and the remote — previous, play/pause, next, volume. OAuth with PKCE, no embedded secret: you create your own Spotify app and paste its client ID. **Sound keeps playing from your Spotify**: no API can stream it into a third-party app. Control needs Premium, display works on free accounts. |
